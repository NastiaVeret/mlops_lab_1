...
def main(cfg: DictConfig):
    ...
    for name, sampler in samplers.items():
        ...
        with mlflow.start_run(run_name=f"Comparison_{name}"):
            study.optimize(lambda trial: objective(trial, X_train, y_train, cfg), n_trials=cfg.hpo.n_trials)
            ...
            plt.plot(range(1, len(best_values) + 1), best_values, label=f"{name} (Best: {study.best_value:.4f})")
